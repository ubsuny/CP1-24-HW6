...
def load_pickle_data(pickle_file_path: str, structure_size: tuple = None):
    ...
    try:
        if not isinstance(pickle_file_path, str) or not os.path.isfile(pickle_file_path):
            err = f"The file '{pickle_file_path}' was not found."
        ...
        # Check if the loaded DataFrame has more than one element
        if len(df.index) == 1 and len(df.columns) == 1:
            print("Warning: The DataFrame only has a single element, which may not match the expected structure.")
        ...
        if structure_size:
            ...
            if len(df.index) == 1 and len(df.columns) == 1:
                print("Warning: The DataFrame shape will be manually set to 2x2.")
                df.index = list(range(0, 2))  # Set to 2 elements (default case)
                df.columns = list(range(0, 2))  # Set to 2 elements (default case)
            else:
                df.index = list(range(0, len(df.index)))  # Use the actual length of index
                df.columns = list(range(0, len(df.columns)))  # Use the actual length of columns
